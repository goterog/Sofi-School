...
export function DashboardShell({ data, configured }: { data: DemoDashboardData | LiveData; configured: boolean }) {
  ...
  return <main className="min-h-screen bg-cloud text-ink">
    ...
    <section className="mx-auto max-w-[1500px] px-4 py-5 sm:px-8 lg:px-10">
        <div className="rounded-lg border border-ink/8 bg-white p-5 shadow-line"><p className="text-sm font-bold uppercase text-teal">{configured ? "Dashboard privado" : "Vista demo local"}</p><h1 className="mt-1 text-2xl font-semibold sm:text-3xl">Hola, {data.profileName}</h1><p className="mt-2 text-sm leading-6 text-ink/60">Portafolio, guía familiar y administración segura en un solo lugar.</p></div>
