...
import { Archive, CalendarDays, ChevronDown, Download, ExternalLink, FileText, Images, Pencil, Search, X } from "lucide-react";

export type PortfolioItem = {
  id: string; student_id: string; student_name: string; area: string; area_id: string; topic: string; topic_id: string;
  title: string; summary: string; observation: string; activity_date: string; evidence_kind: "note" | "photo" | "video" | "document" | "link";
...
const labels = { note: "Nota", photo: "Foto", video: "Video", document: "Documento", link: "Enlace" };
...
function EvidenceCard({ entry, areas, topics, onChanged, archived }: { entry: PortfolioItem; areas: Array<{ id: string; name: string }>; topics: Array<{ id: string; area_id: string; name: string }>; onChanged: () => void; archived: boolean }) {
  ...
  async function toggleMedia() {
    ...
    if (!next || media.length || !entry.media_count) return;
    setLoading(true);
    const response = await fetch(`/api/dashboard/portfolio/${entry.id}`);
  ...
  async function handleEdit(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = await patch({ action: "update", title: data.get("title"), summary: data.get("summary"), observation: data.get("observation"), activityDate: data.get("activityDate"), areaId: data.get("areaId"), topicId: data.get("topicId"), privacyNotes: data.get("privacyNotes") });
