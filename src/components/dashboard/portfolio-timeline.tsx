...
import { Archive, CalendarDays, ChevronDown, FileText, Pencil, Play, Search } from "lucide-react";
...
export function PortfolioTimeline({ entries, areas, topics, showArchive = false, toolbarLead, createAction }: {
  entries: PortfolioItem[]; areas: Array<{ id: string; name: string }>; topics: Array<{ id: string; area_id: string; name: string }>; showArchive?: boolean; toolbarLead?: React.ReactNode; createAction?: React.ReactNode;
}) {
...
function EvidenceCard({ entry, areas, topics, onChanged, archived }: { entry: PortfolioItem; areas: Array<{ id: string; name: string }>; topics: Array<{ id: string; area_id: string; name: string }>; onChanged: () => void; archived: boolean }) {
  ...
  return <article className={`rounded-lg border bg-white p-5 shadow-line ${archived ? "border-coral/20" : "border-ink/8"}`}>
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
      <div><p className="text-xs font-bold uppercase text-teal">{entry.area} · {entry.topic}</p><h3 className="mt-1 text-xl font-semibold">{entry.title}</h3></div>
    ...
    {!archived && entry.media_count ? <div className="mt-4 grid gap-2">{mediaLoading ? <div className="flex h-52 items-center justify-center rounded-lg bg-cloud text-sm text-ink/55">Preparando vista privada…</div> : media.map((item) => <MediaPreview key={item.id} item={item} />)}{!mediaLoading && !media.length ? <p className="rounded-lg bg-cloud p-4 text-sm text-ink/55">Los anexos ya no están disponibles.</p> : null}</div> : null}
    <p className="mt-3 text-xs text-ink/48">Publicado por {entry.author_name}</p>
    ...
    {archived ? <div className="mt-4"><span className="rounded bg-coral/10 px-2 py-1 text-xs font-semibold text-coral">Archivada</span></div> : null}
...
function MediaPreview({ item }: { item: MediaItem }) {
  ...
  return <div className="overflow-hidden rounded-lg bg-cloud">{image && item.signedUrl ? <a href={item.signedUrl} target="_blank" rel="noreferrer"><Image unoptimized src={item.signedUrl} alt={item.caption || item.original_name || "Evidencia privada"} width={800} height={500} className="h-64 w-full object-cover transition hover:scale-[1.01]" /></a> : pdf && item.signedUrl ? <iframe src={`${item.signedUrl}#page=1&toolbar=0&navpanes=0`} title={item.original_name || "Vista previa PDF"} loading="lazy" className="h-64 w-full bg-white" /> : embedUrl ? <iframe src={embedUrl} title={`Vista previa de ${item.external_provider}`} loading="lazy" allow="encrypted-media; picture-in-picture" allowFullScreen className="h-64 w-full bg-ink" /> : item.signedUrl ? <a href={item.signedUrl} target="_blank" rel="noreferrer" className="flex h-56 flex-col items-center justify-center gap-3 bg-gradient-to-br from-teal/10 to-forest/10"><Play className="h-11 w-11 text-forest" /><span className="font-semibold text-forest">Abrir {item.external_provider || "evidencia"}</span></a> : <div className="flex h-40 items-center justify-center"><FileText className="h-10 w-10 text-teal" /></div>}</div>;
}
